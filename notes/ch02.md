...
| 方法（$N_d=2$，4-bit 凍結權重 3.7 GiB） | 每卡 | 相對 baseline 省下 |
|---|---:|---:|
| baseline（單卡 4-bit LoRA） | 3.7 + 0.135 = **3.84 GiB** | — |
| ZeRO-1 | 3.7 + 0.085 = **3.79 GiB** | 0.051 GiB（**1.3%**） |
| ZeRO-2 | 3.7 + 0.076 = **3.78 GiB** | 0.059 GiB（**1.5%**） |
| ZeRO-3 / FSDP（bf16 base） | 13.9/2 + 0.068 = **7.02 GiB** | 切參數才有質變 |

**兩個要跟老闆講的結論：**

1. **ZeRO-1/2 對 LoRA 幾乎無效**，因為凍結的 3.7 GiB 基礎權重根本不在它們的切分範圍內。
...
|---|---|---:|---:|---:|---|
| **本機 M4 Pro 24GB（MLX）** | bs=2 是 12.1 GiB；bs=4 就 19.4 GiB 貼邊（logits 隨 bs 線性） | 2 | 16 | 1 | 基準 1.0× |
